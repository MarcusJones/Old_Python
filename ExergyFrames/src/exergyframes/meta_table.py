...
import unittest
from config import *
from utility_excel import ExcelBookAPI, ExcelBookRead
import os
...
def getDescriptionsOut(descriptionsFilePath):
    ...
    descriptionsTable = parametersBook.getTable("Descriptions",1,None,0,None)
...
def getDescriptionsBal(descriptionsFilePath,sheetName):
    ...
    descriptionsTable = parametersBook.getTable(sheetName,1,200,1,4)
    descriptions = list()
    for row in descriptionsTable[1:]:
        if row[0]:
            try:
...
def getDescriptionsZoneNames(descriptionsFilePath,sheetName):
    # descriptionsFilePath is an excel file with a proper "Descriptions" sheet
    parametersBook = ExcelBookAPI(descriptionsFilePath)
    descriptionsTable = parametersBook.getTable(sheetName,1,200,1,2)
    descriptions = list()
    for row in descriptionsTable[1:]:
        if row[0]:
            try:
...
def getSearchIndexes(excelFilePath):
    ...
    sheetName = "Search1"
    searchIdxTable = wb.getTableAll(sheetName)
