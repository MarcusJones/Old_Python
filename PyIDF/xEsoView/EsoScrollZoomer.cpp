...
// $Id: EsoScrollZoomer.cpp,v 1.7 2006/11/17 21:48:33 cschiefer Exp $
...
public:
	ScrollData() 
		: scrollBar(NULL), position(EsoScrollZoomer::OppositeToScale),
	...
	EsoScrollBar* scrollBar;
	EsoScrollZoomer::ScrollBarPosition position;
#if QT_VERSION < 0x040000
...
EsoScrollZoomer::EsoScrollZoomer(QwtPlotCanvas* canvas)
	: QwtPlotZoomer(canvas), d_cornerWidget(NULL), d_hScrollData(NULL), d_vScrollData(NULL), d_inZoom(false)
{
	if (!canvas)
		return;
...
EsoScrollZoomer::~EsoScrollZoomer()
{
...
void EsoScrollZoomer::rescale()
{
	qDebug("EsoScrollZoomer::rescale()");
...
EsoScrollBar* EsoScrollZoomer::scrollBar(Qt::Orientation o)
{
	EsoScrollBar*& sb = (o == Qt::Vertical) ? d_vScrollData->scrollBar : d_hScrollData->scrollBar;

	if (sb == NULL)
	{
		sb = new EsoScrollBar(o, canvas());
		sb->hide();
		connect(sb, SIGNAL(valueChanged(Qt::Orientation, float, float)), SLOT(scrollBarMoved(Qt::Orientation, float, float)));
	}
	return sb;
}

EsoScrollBar* EsoScrollZoomer::horizontalScrollBar() const
{
	return d_hScrollData->scrollBar;
}

EsoScrollBar* EsoScrollZoomer::verticalScrollBar() const
{
	return d_vScrollData->scrollBar;
}

#if QT_VERSION < 0x040000
void EsoScrollZoomer::setHScrollBarMode(QScrollView::ScrollBarMode mode)
#else
void EsoScrollZoomer::setHScrollBarMode(Qt::ScrollBarPolicy mode)
#endif
{
...
#if QT_VERSION < 0x040000
void EsoScrollZoomer::setVScrollBarMode(QScrollView::ScrollBarMode mode)
#else
void EsoScrollZoomer::setVScrollBarMode(Qt::ScrollBarPolicy mode)
#endif
{
...
#if QT_VERSION < 0x040000
QScrollView::ScrollBarMode EsoScrollZoomer::hScrollBarMode() const
#else
Qt::ScrollBarPolicy EsoScrollZoomer::hScrollBarMode() const
#endif
{
...
#if QT_VERSION < 0x040000
QScrollView::ScrollBarMode EsoScrollZoomer::vScrollBarMode() const
#else
Qt::ScrollBarPolicy EsoScrollZoomer::vScrollBarMode() const
#endif
{
	return d_vScrollData->mode;
}

void EsoScrollZoomer::setHScrollBarPosition(ScrollBarPosition pos)
{
...
void EsoScrollZoomer::setVScrollBarPosition(ScrollBarPosition pos)
{
...
EsoScrollZoomer::ScrollBarPosition EsoScrollZoomer::hScrollBarPosition() const
{
	return d_hScrollData->position;
}

EsoScrollZoomer::ScrollBarPosition EsoScrollZoomer::vScrollBarPosition() const
{
	return d_vScrollData->position;
}

void EsoScrollZoomer::setCornerWidget(QWidget* w)
{
	if (w != d_cornerWidget)
	{
		if (canvas())
...
QWidget* EsoScrollZoomer::cornerWidget() const
{
	return d_cornerWidget;
}

bool EsoScrollZoomer::eventFilter(QObject* o, QEvent* e)
{
	if (o == canvas())
	{
		switch (e->type())
			...
			case QEvent::Resize:
				{
					...
					rect.setRect(rect.x() + fw, rect.y() + fw, rect.width() - 2 * fw, rect.height() - 2 * fw);
...
bool EsoScrollZoomer::needScrollBar(Qt::Orientation o) const
{
#if QT_VERSION < 0x040000
	QScrollView::ScrollBarMode mode;
#else
	Qt::ScrollBarPolicy mode;
#endif
	float zoomMin, zoomMax, baseMin, baseMax;
...
void EsoScrollZoomer::updateScrollBars()
{
	if (!canvas())
		return;
	...
	if (showHScrollBar)
	{
		EsoScrollBar* sb = scrollBar(Qt::Horizontal);
	...
	if (showVScrollBar)
	{
		EsoScrollBar* sb = scrollBar(Qt::Vertical);
...
void EsoScrollZoomer::layoutScrollBars(const QRect& rect)
{
	int hPos = xAxis();
	if (hScrollBarPosition() == OppositeToScale)
	...
	EsoScrollBar* hScrollBar = horizontalScrollBar();
	EsoScrollBar* vScrollBar = verticalScrollBar();
...
void EsoScrollZoomer::scrollBarMoved(Qt::Orientation o, float min, float)
{
	if (o == Qt::Horizontal)
...
int EsoScrollZoomer::oppositeAxis(int axis) const
{
	switch (axis)
	{
...
/*!
   ...
   \brief Return the date label for a position - overwrite original method

 */
QwtText EsoScrollZoomer::trackerText(const QwtDoublePoint& pos) const
{
	//QwtDoublePoint pt = invTransform(pos);//transform pixel into plot coordinates
	...
	switch (rubberBand())
	{
		...
		case VLineRubberBand:
			{
				QDateTime actTime = this->m_startTime.addSecs((int) pos.x());
				return actTime.toString("ddd MMM dd hh:mm");
			}
		default:
			QDateTime actTime = this->m_startTime.addSecs((int) pos.x());
			return QwtText(actTime.toString("ddd MMM dd hh:mm") + QString().sprintf(", %.4f  ", pos.y()));
	}
